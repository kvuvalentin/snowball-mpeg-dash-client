...
void
g_list_push_allocator (gpointer dummy)
...
static inline GList *
_g_list_remove_link (GList * list, GList * link)
{
  if (link)
    {
      if (link->prev)
 ...
 */
GList *
g_list_nth_prev (GList * list, guint n)
{
  while ((n-- > 0) && list)
    list = list->prev;
...
g_list_insert_sorted_real (GList * list,
			   gpointer data, GFunc func, gpointer user_data)
{
  GList *tmp_list = list;
  GList *new_list;
  gint cmp;
...
g_list_sort_merge (GList * l1,
		   GList * l2, GFunc compare_func, gpointer user_data)
{
  GList list, *l, *lprev;
  gint cmp;
